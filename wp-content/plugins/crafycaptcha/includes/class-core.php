...
class CrafyCaptcha_Core
{
    ...
    public static function get_options()
    {
        ...
        if (!is_array($json) || !isset($json['security']) || !is_string($json['security'])) {
            wp_send_json_error(array('message' => 'Invalid or malformed request.'));
        }

        $nonce = sanitize_text_field($json['security']);
        if (!wp_verify_nonce($nonce, 'crafycaptcha_options_nonce')) {
            wp_send_json_error(array('message' => 'Invalid or expired CSRF token.'));
        }

        $crafy = self::get_instance();

        if (!$crafy) {
            wp_send_json_error(array('message' => 'CrafyCAPTCHA is not configured correctly.'));
        }
        ...
        try {
            ...
            wp_send_json_error(array('message' => 'Error creating the security flow.'));
        }
    }
    ...
    public static function is_token_valid()
    {
        ...
        if ($raw_token === null) {
            ...
            // phpcs:ignore WordPress.Security.NonceVerification.Missing
            // self::log( '$_POST keys: ' . implode(', ', array_map('sanitize_text_field', array_keys($_POST))) );
            return false;
        }
